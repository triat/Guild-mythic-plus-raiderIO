...
final class GMPR_Renderer {
	...
	public static function render_guild_table(array $data, bool $is_stale): string {
		...
		$out .= '<thead><tr>';
		$out .= '<th scope="col">' . esc_html__('Member', 'gmpr') . '</th>';
		...
		foreach ($members as $m) {
			...
			$out .= '<tr>';
			$out .= '<td data-label="' . esc_attr__('Member', 'gmpr') . '">' . esc_html($name) . '</td>';
			$out .= '<td data-label="' . esc_attr__('Score Mythic+', 'gmpr') . '" class="gmpr-col-score">' . esc_html($score) . '</td>';
			if ($url !== '') {
				$out .= '<td data-label="' . esc_attr__('Raider.IO', 'gmpr') . '"><a href="' . esc_url($url) . '" target="_blank" rel="noopener noreferrer">' . esc_html__('Profile', 'gmpr') . '</a></td>';
			} else {
		...
		if (count($members) === 0) {
			$out .= '<tr><td colspan="3">' . esc_html__('No members found.', 'gmpr') . '</td></tr>';
		}
